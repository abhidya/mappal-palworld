...
public static class Extract
{
    ...
    public class Rec
    {
        ...
        // pose bake
        public string anim; public float animFrame;
        public int bones, bonesMatched, vertsSkinned;
        public float[] bind_min_cm, bind_max_cm;
    }

    // Set when a pose bake was requested; shared across every mesh in the run.
    static Pose.Baker Baker;
    ...
    {
        var outDir = args.Length > 1 ? args[1] : Path.Combine(P.Base, "meshes_uv");
        ...
        var manifestFile = args.Length > 2 ? args[2] : Path.Combine(P.Base, "mesh_manifest.json");
        ...
        // name -> game path, deduped (many types share a mesh)
        var targets = new Dictionary<string, string>();
        foreach (var (_, e) in manifest)
        {
            if (!e.TryGetProperty("meshPath", out var mp)) continue;
            var gp = mp.GetString();
            if (string.IsNullOrEmpty(gp)) continue;
            targets[gp[(gp.LastIndexOf('/') + 1)..]] = gp;
        }
        Console.Error.WriteLine($"targets={targets.Count}");

        // Optional pose bake: args[3]=AnimSequence asset name, args[4]=frame.
        // Skinned meshes get one frame of that sequence baked into their vertex
        // positions; static meshes are unaffected.
        if (args.Length > 3 && !string.IsNullOrEmpty(args[3]))
        {
            var frame = args.Length > 4 ? float.Parse(args[4], System.Globalization.CultureInfo.InvariantCulture) : 0f;
            Baker = new Pose.Baker(provider, args[3], frame);
            Console.Error.WriteLine($"pose: {Baker.AnimPath} frame={Baker.Frame}/{Baker.NumFrames}");
        }

        var results = new List<Rec>();
        int i = 0;
        foreach (var (name, gp) in targets.OrderBy(k => k.Key))
        {
            if (++i % 25 == 0) Console.Error.WriteLine($"  {i}/{targets.Count}");
            var r = new Rec { name = name, meshPath = gp };
        ...
        File.WriteAllText(Path.Combine(P.Base, Path.GetFileNameWithoutExtension(manifestFile) + "_uv_report.json"),
    ...
    static void One(CUE4Parse.FileProvider.DefaultFileProvider provider, Rec r, string outDir, string texDir)
    {
        ...
        if (r.kind == "SK")
        {
            ...
            var vpos = lod.Vertices.Select(v => v.Position).ToArray();
            r.bones = dto.Bones.Length;
            if (Baker != null)
            {
                r.bind_min_cm = MinV(Flat3(vpos)); r.bind_max_cm = MaxV(Flat3(vpos));
                r.anim = Baker.AnimName; r.animFrame = Baker.Frame;
                vpos = SkinPositions(lod.Vertices, dto.Bones, r);
            }
            pos = Flat3(vpos);
        ...
        for (int s = 0; s < mats.Length; s++)
        {
            ...
            try
            {
                if (mi != null && mi.TryLoad(out var mo) && mo is UMaterialInterface umat)
                {
                    ...
                    if (pick.tex == null && p.Textures.Count == 0)
        ...
        r.glb = Path.GetRelativePath(P.Base, glbPath);
        r.ok = true;
    }

    /// Linear-blend skinning against the baked animation frame. Vertices with no
    /// influence, or whose bones the animation skeleton does not carry, keep
    /// their bind position — never nudged by hand.
    /// Normals are not transformed here: WriteGlb recomputes them from the
    /// final geometry, which is exact for the posed mesh.
    static CUE4Parse.UE4.Objects.Core.Math.FVector[] SkinPositions(
        SkinnedMeshVertex[] verts, MeshBoneDto[] bones, Rec r)
    {
        var mats = Baker.SkinMatrices(bones, out var matched);
        r.bonesMatched = matched;
        var outPos = new CUE4Parse.UE4.Objects.Core.Math.FVector[verts.Length];
        int skinned = 0;
        for (int i = 0; i < verts.Length; i++)
        {
            ...
            var src = v.Position;
            float wsum = 0f;
            var acc = new CUE4Parse.UE4.Objects.Core.Math.FVector(0, 0, 0);
            foreach (var inf in v.Influences)
            {
                if (inf.Bone >= mats.Length) continue;
                var m = mats[inf.Bone];
                if (m == null) continue;
                var p = Pose.XformPos(m, src);
                acc.X += p.X * inf.Weight; acc.Y += p.Y * inf.Weight; acc.Z += p.Z * inf.Weight;
                wsum += inf.Weight;
            }
            if (wsum > 1e-4f) { outPos[i] = acc / wsum; skinned++; }
            else outPos[i] = src;
        }
        r.vertsSkinned = skinned;
        return outPos;
    }
